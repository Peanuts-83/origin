...
sorted_by_name = sorted(people, key=lambda k: k['name'].lower())
...
name_declarations = list(map(lambda k: f"{k['name']} is {k['age']} years old", sorted_by_name))
...
under_seventy = sorted(list(filter(lambda k: k['age'] < 70, sorted_by_name)), key=lambda d: d['age'])
